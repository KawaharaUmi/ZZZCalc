        ...
        for(const val of define.skillType) {
            ...
            for(const skillData of groupArray) {
                ...
                for(const data of result.params) {
                    const level = this.skills[result.group.primary].level
                    const damage = {
                        parent: result,
                        level: level,
                        get normal() {
                            return this.parent._root.calcDmg(data, this.level, 'normal')
                        },
                        get critical() {
                            return this.parent._root.calcDmg(data, this.level, 'critical')
                        },
                        get expected() {
                            return this.parent._root.calcDmg(data, this.level, 'expected')
                        }
                    }
